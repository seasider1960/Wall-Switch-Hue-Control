 ...
 /*
 To provide a hardware-based "soft switch" for the Hue lights using exisitng 3-way and 4-way wall switches and existing wiring
 Allows Hue lights to be always available for app and voice control while retaining convenience of physical switches
 Adapted from code by By James Bruce (MakeUseOf.com) at https://gist.github.com/jamesabruce/8bc9faf3c06e30c3f6fc who in turn adapted from
code by Gilson Oguime. https://github.com/oguime/Hue_W5100_HT6P20B/blob/master/Hue_W5100_HT6P20B.ino
 */
...
const char hueHubIP[] = "XXX.XXX.X.XXX";  // Hue hub IP
const char hueUsername[] = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXX";  // Hue developer name
const int hueHubPort = 80;

// Local network details

const char ssid[] = "XXXXXXXXXXXXXXXXXXXXXXXX";  //  network SSID (name)
const char pass[] = "XXXXXXXXX";       // network password

// Other

int lightNum = 1; // Kitchen downlight 8 - arbitary light from kitchen group to check on/off status of all group lights
int switchPin = 13; // -- control sense pin, mapped to D7 on NodeMCU
...
void loop() 
  {
     if(millis() % getHueTime == 0) //checks light state every 2s
      {
        getHue(1);
      }
     ...
     if (digitalRead(switchPin) != previousState && millis() - stateChangeTime > stateLock) // state change (multiple changes within 2.5s period ignored)
        {
          onOffCommandSent = false; //avoids mutiple commands to bridge
          previousState = state;
          stateChangeTime = millis();
          if (onOffState == false) // if lights are off, send "on" command
           {
             ...
             onOffCommandSent = true; // avoids multiple commands on one state change
