...
int groupNum = 8; // Kitchen group -- number assigned by Hue bridge
            ...
            if (onOffState == false) // if lights are off, send "on" command
             {
               ...
               setHue(groupNum,command); //  Kitchen Group (downlights and lightstrips) comprise:
...
void setHue(int groupNum,String command) // setHue() passes the on/off commands in JSON format and makes a HTTP PUT request to the bridge
  {
    if (client.connect(hueHubIP, hueHubPort))
     {
        client.print("PUT /api/<Hue Developer Name>/groups/8/action HTTP/1.1"); //statements combined to improve performance
        //client.print(hueUsername);
        //client.print("/groups/"); //NOTE: if addressing individual lights change "groups" to "lights"
        //client.print(groupNum); 
        //client.println("/action HTTP/1.1"); //NOTE: if addressing individual lights change "action" to "state"
